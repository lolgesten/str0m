...
pub struct ProbeClusterConfig {
    ...
    /// generate padding bursts more frequently than this.
...
impl ProbeClusterState {
    ...
    /// `now >= start_time + (bytes_sent / target_bitrate)`. Explicitly capped
    /// probes drain each requested burst at one instant, retaining the rate and
    /// minimum spacing constraints between bursts for coarse event-loop timers.
    pub fn next_probe_time(&self) -> Instant {
        if self.config.is_capped() && self.burst_remaining > DataSize::ZERO {
            return self.last_padding_at;
        }
        self.next_padding_time()
    }

    /// Deadline for generating another burst, even if the preceding request was underfilled.
    pub(crate) fn next_padding_time(&self) -> Instant {
    ...
    pub fn should_send_now(&self, now: Instant) -> bool {
        let next_time = self.next_padding_time();
        let min_burst_time = self.last_padding_at + self.config.min_probe_delta();
    ...
    pub fn next_packet(&mut self, now: Instant) -> Option<DataSize> {
        ...
        let bytes_remaining =
            if self.config.is_capped() && self.packets_sent < self.config.min_packet_count {
        ...
        if request_bytes == DataSize::ZERO {
            ...
            self.burst_remaining = request_bytes;
            Some(request_bytes)
        }
    }
    ...
    pub fn record_packet(&mut self, now: Instant, size: DataSize) {
        ...
        // treat that as the start of the probe for subsequent byte-rate accounting.
        if self.started_at.is_none() {
            self.started_at = Some(now);
        }

        self.bytes_sent += size;
        self.burst_remaining = self.burst_remaining.saturating_sub(size);
        self.packets_sent += 1;
...
mod test {
    ...
    #[test]
    fn uncapped_probe_keeps_original_byte_completion() {
        let now = Instant::now();
        let mut probe = ProbeClusterState::new(ProbeClusterConfig::new(
            1.into(),
            Bitrate::kbps(32),
            ProbeKind::Initial,
        ));
        let first = probe.next_packet(now).unwrap();
        probe.record_packet(now, first);
        assert!(!probe.is_complete(now));
        assert!(probe.next_packet(now + Duration::from_secs(1)).is_none());
    }
